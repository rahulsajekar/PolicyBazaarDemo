...
public class TestingTermLifeInsurance {
	...
	public void testTermLifeInsurance(String testID, String gender, String name, String dob, String mobileNO,
			...
			String pinCode, String residentStatus,String expectedPremium) {
		...
		f.generateReport(testID, actualPreiumAmount, expectedPremium);
		Assert.assertEquals(actualPreiumAmount, expectedPremium);
		...
		try {
			...
			for (int i = 1; i <= totalRows; i++) {
				...
				for (int j = 0; j < totalCols; j++) {
					...
					} catch (Exception e) { // convert numeric data to string
						cellData = Double.toString(currentCell.getNumericCellValue());
					}
